...
public class MrReduceServer {
    ...
    static class MrReduceServerImpl extends AssignJobGrpc.AssignJobImplBase {
        ...
        @Override
        public void reduce(ReduceInput request, StreamObserver<ReduceOutput> responseObject) {
            try {
                mr.reduce(request.getInputfilepath(), request.getOutputfilepath());
                responseObject.onNext(ReduceOutput.newBuilder().setJobstatus(2).build());
            } catch (Exception e) {
                e.printStackTrace();
                responseObject.onNext(ReduceOutput.newBuilder().setJobstatus(1).build());
            }
